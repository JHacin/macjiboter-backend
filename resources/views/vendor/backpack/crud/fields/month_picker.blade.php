...
{{-- bootstrap datepicker input, copied from Backpack date_picker & tweaked to work better with months due to how BS picker "format" works --}}
...
$field['datetime_picker_options']['language'] = $field['datetime_picker_options']['language'] ?? \App::getLocale();
?>
...
@push('crud_fields_scripts')
    ...
    @if ($field['datetime_picker_options']['language'] !== 'en')
        @loadOnce('packages/bootstrap-datepicker/dist/locales/bootstrap-datepicker.'.$field['datetime_picker_options']['language'].'.min.js')
    @endif
    ...
    <script>
        ...
        function bpFieldInitDatePickerElement(element) {
            const displayFormatter = new Intl.DateTimeFormat('sl', { dateStyle: 'full' });
            ...
            $customConfig = $.extend({
                format: {
                    toDisplay: (date) => {
                        const dateParts = displayFormatter.formatToParts(date);
            ...
            var $existingVal = $field.val();
                ...
                if( $existingVal && $existingVal.length ){
                    ...
                    var day = parts[2];
                    preparedDate = $customConfig.format.toValue(new Date(year, month, day));
